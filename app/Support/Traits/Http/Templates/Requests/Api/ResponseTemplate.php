<?php

declare(strict_types=1);

namespace App\Support\Traits\Http\Templates\Requests\Api;

use App\Exceptions\Json\HttpJsonException;
use Illuminate\Database\QueryException;
use Illuminate\Http\JsonResponse;
use Illuminate\Http\Response;
use Throwable;

trait ResponseTemplate
{
    /**
     * Standard JSON success response format.
     *
     * @param int $status
     * @param string $message
     * @param array $data
     * @param array $headers
     * @param int $options
     * @return JsonResponse
     */
    public function successResponse(
        int $status = Response::HTTP_OK,
        string $message = '',
        array $data = [],
        array $headers = [],
        int $options = 0
    ): JsonResponse {
        $data = [
            'message' => empty($message)
                ? __('shared.common.success')
                : period_at_the_end($message),
            'data' => $data,
        ];

        return response()->json($data, $status, $headers, $options);
    }

    /**
     * Standard JSON no content response format.
     *
     * @param array $headers
     * @return JsonResponse
     */
    public function noContentResponse(array $headers = []): JsonResponse
    {
        return response()->json([], Response::HTTP_NO_CONTENT, $headers);
    }

    /**
     * Standard JSON error response format.
     *
     * @param int $status
     * @param string $errorCode
     * @param string $message
     * @param array $errors
     * @param array $headers
     * @param int $options
     * @return JsonResponse
     */
    public function errorResponse(
        int $status,
        string $errorCode = '',
        string $message = '',
        array $errors = [],
        array $headers = [],
        int $options = 0
    ): JsonResponse {
        return response()->json([
            'error_code' => empty($errorCode) // Use well-formed error code response (such as InvalidKey, BadKey, UnknownKey) to indicate and provide more details around the error.
                ? studly(Response::$statusTexts[$status])
                : studly($errorCode),
            'message' => empty($message)
                ? __('shared.common.error')
                : period_at_the_end($message),
            'errors' => $errors,
        ], $status, $headers, $options);
    }

    /**
     * HTTP request error response format.
     *
     * @param HttpJsonException $exception
     * @return JsonResponse
     */
    public function httpErrorResponse(HttpJsonException $exception): JsonResponse
    {
        return $this->errorResponse(
            status: $exception->getStatus(),
            errorCode: $exception->getErrorCode(),
            message: $exception->getMessage(),
            errors: $exception->getErrors()
        );
    }

    /**
     * Database query internal server error response format.
     *
     * @param QueryException $exception
     * @return JsonResponse
     */
    public function queryErrorResponse(QueryException $exception): JsonResponse
    {
        $status = Response::HTTP_INTERNAL_SERVER_ERROR;
        $description = 'The server encountered an internal error or misconfiguration.';

        if (config('app.debug')) {
            [, , $description] = filled($exception->errorInfo) ? $exception->errorInfo : [null, null, $exception->getMessage()];
        }

        return $this->errorResponse(
            status: $status,
            errorCode: studly(Response::$statusTexts[$status]),
            message: __('shared.query.' . $status),
            errors: ['server' => [period_at_the_end($description)]]
        );
    }

    /**
     * Server error response format.
     *
     * @param Throwable $exception
     * @return JsonResponse
     */
    public function serverErrorResponse(Throwable $exception): JsonResponse
    {
        $status = Response::HTTP_INTERNAL_SERVER_ERROR;

        return $this->errorResponse(
            status: $status,
            errorCode: studly(Response::$statusTexts[$status]),
            message: __('shared.http.' . $status),
            errors: config('app.debug')
                ? ['server' => [period_at_the_end($exception->getMessage())]]
                : ['server' => ['An unexpected error occurred. Please try again later.']]
        );
    }
}
